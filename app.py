...
@app.route('/')
def home():
    return 'You\'re at the home page'

@app.route('/api/v1.0/precipitation')
def precipitation():
    dateList = [int(el) for el in max(session.query(Measurement.date).all()).date.split('-')]
    earliestDate = dt.date(*dateList) - dt.timedelta(days=365)
    results = session.query(Measurement.tobs, Measurement.date).filter(Measurement.date > earliestDate)
    temperatures = {}
